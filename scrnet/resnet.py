...
class RCNet(nn.Module):
    ...
    def forward(self, x, modal):
        ...
        residual = out
        out = self.pre_attn_norm(out.permute(0, 2, 1)).permute(0, 2, 1)  # 归一化通道维度
        out = self.attention(out, modal)
        out = self.self_attention(out)
        out = self.post_attn_norm(out.permute(0, 2, 1)).permute(0, 2, 1)  # 归一化通道维度
        ...
        c = torch.sigmoid(self.dense2(out))  # 置信度分数
        out = self.dense(out)  # 分类 logits
